...
PLATFORM_SCHEMA = PLATFORM_SCHEMA.extend(
    {
        ...
        vol.Optional(CONF_SEND_LEVELS_ON_STARTUP, default=False): cv.boolean,
    }
)
...
def async_setup_platform(hass, config, async_add_devices, discovery_info=None):
    ...
    dmx_gateway = DMXGateway(
        host, universe, port, overall_default_level, config[CONF_DMX_CHANNELS]
    )

    lights = (
        DMXLight(light, dmx_gateway, send_levels_on_startup, default_light_type)
...
class DMXLight(LightEntity):
    ...
    def __init__(self, light, dmx_gateway, send_immediately, default_type):
        ...
        self._dmx_gateway.set_channels(
            self._channels, self.dmx_values, send_immediately
        )
